...
cd $HOME;sudo curl -k -L -O --retry 333 https://raw.githubusercontent.com/gjwdyk/NGINX-Notes/main/NMS-Instance-Manager/N%2BLB/LBnginx.conf;sudo chmod 666 $HOME/LBnginx.conf;/bin/bash $HOME/EditVariable.sh $HOME/LBnginx.conf $HOME/EnvironmentVariable.sh;sudo mv $HOME/LBnginx.conf /etc/nginx/nginx.conf;sudo nginx -t;sudo nginx -s reload
cd $HOME;sudo curl -k -L -O --retry 333 https://raw.githubusercontent.com/gjwdyk/NGINX-Notes/main/NMS-Instance-Manager/N%2BLB/LBdefault.conf;sudo chmod 666 $HOME/LBdefault.conf;/bin/bash $HOME/EditVariable.sh $HOME/LBdefault.conf $HOME/EnvironmentVariable.sh;sudo mv $HOME/LBdefault.conf /etc/nginx/conf.d/default.conf;sudo nginx -t;sudo nginx -s reload
