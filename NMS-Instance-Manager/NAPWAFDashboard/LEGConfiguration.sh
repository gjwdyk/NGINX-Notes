...
# LogStash configuration (i.e. input, filter, output) was inserted during LogStash installation, before the LogStash service started.
...
# Change Domain Name reference to proper ElasticSearch instance
sudo sed -i "s/elasticsearch\:9200/localhost\:9200/g" grafana/DS-waf-index.json
sudo sed -i "s/elasticsearch\:9200/localhost\:9200/g" grafana/DS-waf-decoded-index.json
